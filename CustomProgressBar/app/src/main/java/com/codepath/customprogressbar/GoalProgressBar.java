...
public class GoalProgressBar extends View {
    ...
    private String text_to_draw = "Hello World";
    ...
    private void init(Context context,AttributeSet attrs) {
        ...
        font_width = textPaint.measureText(text_to_draw);

        font_height = Math.abs(fontMetrics.ascent) + Math.abs(fontMetrics.descent);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int x_text_draw;
        if(progressEndX == getWidth())
        {
           x_text_draw = progressEndX - (int)font_width;
        }
        else if(progressEndX <= font_width)
        {
            x_text_draw = progressEndX ;
        }else
        {
            x_text_draw = progressEndX - (int)(font_width/2);
        }


        canvas.drawText(text_to_draw,x_text_draw,halfHeight - font_height  ,textPaint);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int measuredWidth = reconcileSize(MeasureSpec.getSize(widthMeasureSpec), widthMeasureSpec);

        int  progressHeight =  (int) getGoalIndicatorHeight();

        int finalHeight = progressHeight + (int)font_height +100;
        final int measuredHeight = reconcileSize(finalHeight, heightMeasureSpec);


     /*   int specHeight = MeasureSpec.getSize(heightMeasureSpec);
        specHeight = specHeight +(int) font_height +20;
        int height;
    ...
    public static int reconcileSize(int size, int measureSpec) {

...
public class AppIndexingUtil {
  ...
  public static void setStickers(final Context context, FirebaseAppIndex firebaseAppIndex) {
    try {
      ...
      Task<Void> task =
          firebaseAppIndex.update(indexables.toArray(new Indexable[indexables.size()]));
  ...
  private static Indexable getIndexableStickerPack(List<Indexable> stickers)
    ...
    Indexable.Builder indexableBuilder =
        getIndexableBuilder(
            StickersDataFactory.getAllStickerReference().get(0).getURL(),
  ...
  private static List<Indexable> getIndexableStickers()
      throws IOException, FirebaseAppIndexingInvalidArgumentException {
    List<Indexable> indexableStickers = new ArrayList<>();

    for (int i = 1; i < StickersDataFactory.getAllStickerReference().size(); i++) {
      Indexable.Builder indexableStickerBuilder =
          getIndexableBuilder(
              StickersDataFactory.getAllStickerReference().get(i).getURL(), STICKER_URL_PATTERN, i);
      indexableStickerBuilder
          .put("keywords", "tag1_" + i, "tag2_" + i)
  ...
  private static Indexable.Builder getIndexableBuilder(
      String stickerURL, String urlPattern, int index) throws IOException {
    String url = String.format(urlPattern, index);

    return new Indexable.Builder("StickerPack")

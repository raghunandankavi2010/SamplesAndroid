...
public class MainActivityTest {
    ...
    public void mainActivityTest() {
        ...
        IdlingRegistry.getInstance().register(mActivityTestRule.getActivity().espressoTestIdlingResource);
        ...
        ViewInteraction textView = onView(
                allOf(withId(R.id.tv), withText("Raghunandan Kavi"),
                        childAtPosition(
                                childAtPosition(
                                        withId(android.R.id.content),
                                        0),
                                0),
                        isDisplayed()));
        textView.check(matches(withText("Raghunandan Kavi")));

        appCompatButton.check(matches(not(isEnabled())));

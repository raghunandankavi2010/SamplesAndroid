...
import com.indiainnovates.pucho.presenters.SearchPresenter;
import com.indiainnovates.pucho.screen_contracts.SearchScreen;
...
public class SearchActivity extends AppCompatActivity implements SearchScreen, ShareButtonClickListener {
    ...
    public void onSearchResponse(SearchQueryResponse searchQueryResponse) {
        ...
        if (mLoadMore) {
            Log.i("Load More", "Remove"+true);
            feedAdapter.remove();
            mLoadMore = false;
        }else
        {
            mLoadMore =false;
            Log.i("Load More", "Note Remove"+false);
        }
        ...
        {
           Toast.makeText(SearchActivity.this,"No search results. Try again!.",Toast.LENGTH_SHORT).show();
        }
    }

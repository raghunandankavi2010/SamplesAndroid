...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button signup = (Button) this.findViewById(R.id.button2);
    ...
    public void onEvent(LoginResponseEvent event) {
        ...
        if(event.getLoginResponse().getSuccess()) {
            ...
            Log.i("User_email",""+event.getLoginResponse().getSuccessResponse().getSuccess().getUsername());
            Log.i("User_Name",""+event.getLoginResponse().getSuccessResponse().getSuccess().getFullName());
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.button1:
                /* after singup if user log out user needs to login again */
                loginPresenter.setEmailId(user_name.getText().toString());
                loginPresenter.setPassword(pwd.getText().toString());
                loginPresenter.validateLoginCredentials();
                break;
    ...
    private void handleSignInResult(GoogleSignInResult result) {
        ...
        if (result.isSuccess()) {
            ...
            Log.i(TAG, "Person id" + personId);
            Log.i(TAG, "Token Id " + idToken);
            Log.i(TAG,"Photo Uri" + personPhoto.toString());
    ...
    public void onEvent(LoginViaServerResponseEvent event)
    {
        ...
        if(signupResponse.isSuccess())
        {
            ...
            editor.apply();
            Log.i("User_email",""+signupResponse.getSuccess_response().getEmail());
            Log.i("User_Name",""+signupResponse.getSuccess_response().getFullName());
            Toast.makeText(this.getApplicationContext(),
                    "" + signupResponse.getSuccess_response().getId()
                    , Toast.LENGTH_SHORT).show();
            Intent intent = new Intent(this, QuestionFeed.class);
            startActivity(intent);
            finish();
